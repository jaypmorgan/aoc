...
;; one got sections 5, 6, and 7, while the other also got 7, plus 8
;; and 9.
...
(define test-file "data/day4.test.txt")

(define (read-assignment filepath)
  (map (lambda (line)
	 (map (lambda (l)
		(map
		 string->number          ; convert each to a number
		 (string-split l #\-)))  ; split the lower and upper bound
	      (string-split line #\,)))  ; split each pair
       (read-file-lines filepath)))      ; read the file splitting every line

(define (lower-bound assignment)
  (first assignment))

(define (upper-bound assignment)
  (second assignment))

(define (contained-within? is within)
  (and (>= (lower-bound is) (lower-bound within))
       (<= (upper-bound is) (upper-bound within))))

(define (either-contained-within? is within)
  (or (contained-within? is within)
      (contained-within? within is)))

;; solution to the part one
(length					                        ; get the number of trues
 (filter (lambda (t) t)   			                ; remove all the falses
	 (map (lambda (p) (apply either-contained-within? p))   ; return t/f if either is contained
	      (read-assignment "data/day4.input.txt"))))	; read the assignment

;;;------------------------------- Part 2 -------------------------------

;; generate two lists and check for an intersection of elements.
(define (overlap? p1 p2)
  (not
   (null?
    (intersection
     (range (lower-bound p1) (+ 1 (upper-bound p1)) 1)
     (range (lower-bound p2) (+ 1 (upper-bound p2)) 1)))))

(length					                 ; count the number that overlap
 (filter (lambda (t) t)			                 ; keep only those that overlap
	 (map (lambda (p) (apply overlap? p))            ; is the an overlap
	      (read-assignment "data/day4.input.txt")))) ; read the assignment
