...
;; partition a string into list of lists by a delimiter
;; i.e. "1000\n2000\n3000\n\n4000\n\n5000\n6000\n\n7000\n8000\n9000\n\n10000\n" ->
;; (("1000" "2000" "3000") ("4000") ("5000" "6000") ("7000" "8000" "9000") ("10000"))
;; when partitioning on a newline delimiter
;; partition : str delimiter -> list[list[str]]
(define (partition str delimiter)
  (let ([v (string-split str delimiter)])
    (let loop ([rest v] [parts (list)] [tmp (list)])
      (cond ((null? rest) (reverse parts))
	    ((string= (car rest) "") (loop (cdr rest) (cons (reverse tmp) parts) (list)))
	    (else (loop (cdr rest) parts (cons (car rest) tmp)))))))

;; count the number of calories a single elf is carrying
;; i.e. ("1000" "2000" "3000") -> 6000
;; count-calories : list[str] -> int
(define (count-calories elf)
  (apply + (map string->number elf)))

(define (find-best-elf filepath)
  (let ([cals (map count-calories (partition (read-file filepath) #\newline))])
    (let loop ([elves (cdr cals)] [curr-elf 0] [best-elf 0] [best-cal (car cals)])
      (cond ((null? elves) (values best-elf best-cal))
	    ((> (car elves) best-cal) (loop (cdr elves) (+ curr-elf 1) (+ 1 curr-elf) (car elves)))
	    (else (loop (cdr elves) (+ curr-elf 1) best-elf best-cal))))))
...
;; Find the N best elves from the filepath.
(define (find-n-best filepath n)
  (let ([cals (map count-calories (partition (read-file filepath) #\newline))])
    (take n (sort cals >))))

;; Sum the N best calories from the elves in the filepath
(define (sum-n-best filepath n)
  (apply + (find-n-best filepath n)))
